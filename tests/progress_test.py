...
def test_is_interactive_terminal_ci_env(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test _is_interactive_terminal returns False when CI env var is set."""
    import sys

    from mmgpy._mesh import _is_interactive_terminal

    # Remove pytest from modules temporarily to test CI detection
...
def test_is_interactive_terminal_github_actions(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test _is_interactive_terminal returns False for GITHUB_ACTIONS."""
    import sys
...
def test_is_interactive_terminal_not_tty(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test _is_interactive_terminal returns False when stdout is not a TTY."""
    import io
    import sys
    ...
    # Replace stdout with non-TTY stream
...
def test_resolve_progress_callback_interactive(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test _resolve_progress_callback returns reporter in interactive mode."""
    import sys

    from mmgpy._mesh import _resolve_progress_callback
    from mmgpy._progress import RichProgressReporter
    ...
    try:
        ...
        assert isinstance(callback, RichProgressReporter)
    finally:
        sys.modules.update(original_modules)
