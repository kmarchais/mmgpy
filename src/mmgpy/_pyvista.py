...
def _line_connectivity(
    ...
    if cells_dict is not None and pv.CellType.LINE in cells_dict:
        edges = np.asarray(cells_dict[np.uint8(pv.CellType.LINE)], dtype=np.int32)
...
def _triangle_connectivity_unstructured(
    ...
    if pv.CellType.TRIANGLE not in cells_dict:
        return None
    tris = np.asarray(cells_dict[np.uint8(pv.CellType.TRIANGLE)], dtype=np.int32)
    if len(tris) == 0:
        return None
    return tris
...
def _extract_mmg3d_cells(
    ...
    elements = np.asarray(cells_dict[np.uint8(pv.CellType.TETRA)], dtype=np.int32)
